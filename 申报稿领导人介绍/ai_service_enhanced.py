...
class EnhancedAIService:
    ...
    def call_api(self, prompt: str, json_mode: bool = True, use_file: bool = True, file_id: str = None) -> str:
        ...
        Args:
            ...
            use_file: 是否引用文件ID
            file_id: 文件ID（如果use_file=True且未提供，则使用实例的file_id）
        ...
        if use_file:
            fid = file_id or self._current_file_id
            if fid:
                messages.insert(0, {"role": "system", "content": f"fileid://{fid}"})
    ...
    def extract_data_two_step(
        self, 
        file_id: str, 
        prompt1_path: str = None, 
        prompt2_path: str = None
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            file_id: 上传的文件ID
            prompt1_path: Step1 prompt文件路径（表格转录）
            prompt2_path: Step2 prompt文件路径（规则推理）
        
        Returns:
            {
                "step1_output": "第一步表格转录结果",
                "extracted_data": {...} 第二步结构化数据
            }
        """
        self._current_file_id = file_id
        
        app_dir = get_base_dir()
        default_prompt1 = os.path.join(app_dir, "prompt_step1_table.md")
        default_prompt2 = os.path.join(app_dir, "prompt_step2_rules.md")
        
        p1_path = prompt1_path or default_prompt1
        p2_path = prompt2_path or default_prompt2
        
        prompt1 = self._load_prompt(p1_path)
        prompt2 = self._load_prompt(p2_path)
        
        logger.info(f"Step1: 表格转录 (prompt: {len(prompt1)} chars)")
        tables = self.call_api(prompt1, json_mode=False, use_file=True, file_id=file_id)
        logger.info(f"Step1 完成，输出 {len(tables)} chars")
        
        logger.info(f"Step2: 规则推理 (prompt: {len(prompt2)} chars)")
        step2_prompt = f"以下是本文档的表格数据：\n\n{tables}\n\n{prompt2}"
        json_response = self.call_api(step2_prompt, json_mode=True, use_file=False)
        
        extracted_data = self._parse_and_validate_json(json_response)
    ...
    def delete_file(self, file_id: str) -> bool:
        ...
        try:
            ...
            error_str = str(e)
            if "404" in error_str or "No such File" in error_str:
                logger.debug(f"文件已不存在，无需删除: {file_id}")
                return True
