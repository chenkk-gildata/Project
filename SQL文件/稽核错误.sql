...
WHERE  (YWX = 'A股-特别组'  OR YWX = 'A股-衍生组')
	AND SJJHRQ BETWEEN '2026-1-1' AND '2026-12-31'
	AND SCSFJS='是'
    AND WTLX != '规则歧义'
ORDER BY 2,3,5
...
FROM  [10.101.1.144].FSCSJ.dbo.usrNBSYJHCWTB
WHERE  (YWX = 'A股-特别组'  OR YWX = 'A股-衍生组')
	AND SJJHRQ BETWEEN '2026-5-1' AND '2026-05-30'
...
SELECT PTID,标识,SJJHRQ,表名,SJDM,GGRQ,问题数量,问题分类,问题责任人,备注 FROM #JHCWTB ORDER BY PTID

SELECT 表名,错误原因,问题分类,是否有避免措施,措施,备注 FROM #JHCWTB ORDER BY 1

SELECT * FROM #JHCWTB WHERE 表名 IN ('usrZYLDRCG','usrGSJYLDRBC') AND 问题责任人!='陈凯凯' ORDER BY 1
...
WHERE  (YWX = 'A股-特别组'  OR YWX = 'A股-衍生组')
    AND SCSFJS='是'
    AND WTLX != '规则歧义'
	AND SJJHRQ BETWEEN '2026-03-05' AND '2026-04-01'
ORDER BY 1
