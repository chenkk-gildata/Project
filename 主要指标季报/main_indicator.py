...
class EnhancedDataProcessor:
    ...
    def process_file_with_uploaded_id(self, file_id: str, filename: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                log_dir = os.path.join(script_dir, "主要指标三季报小程序比对", "logs")
    ...
    def _compare_data_with_keys(self, ai_datas: Union[Dict[str, Any], List[Dict[str, Any]]],
        ...
        # 将SQL数据按日期标志和截止日期分组 - 移到循环外部，只执行一次
    ...
    def generate_report(self, results: List[Dict[str, Any]], report_file: str = None) -> str:
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # 确保report文件夹存在
        report_dir = os.path.join(script_dir, "主要指标三季报小程序比对", "report")
        ...
        if not report_file:
            ...
            report_file = os.path.join(report_dir, f"主要指标三季报比对报告_{timestamp}.xlsx")
            ...
            if choice == "1":
                ...
                if results:
                    ...
                    report_file = os.path.join(report_dir,
                                               f"主要指标三季报比对报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
