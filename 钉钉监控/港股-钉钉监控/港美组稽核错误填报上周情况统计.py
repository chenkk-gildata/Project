...
def team_errors_query():
    ...
    sql1 = """
        ...
        WHERE
        A.YWX = '港股'
        AND CAST(A.JHRQ AS date) BETWEEN CAST(GETDATE() - 8 AS date) AND CAST(GETDATE() - 1 AS date)
        GROUP BY
        YWRY
    """
...
def message_trans(result):
    ...
    if sql_result1:
        header1 = '## <font color=#9A32CD>港美组-上周错误情况统计</font>\n'
        body += header1
        body += f'>  <font color=#9A32CD>统计区间： {sql_result2[0][1]} 至 {sql_result2[0][0]}</font>\n'
        his_calc = 0
        new_calc = 0
        ...
        body += f'\n<font color=#9A32CD>【合计】昨日日增错误：{new_calc}条；历史错误：{his_calc}条</font>\n'
        ...
        body += '## <font color=#9A32CD>打的赢，霸得蛮，高质量发展！</font>\n'

    else:
        header1 = '## <font color=#9A32CD>港美组-上周错误情况统计</font>\n'
        body += header1
        body += f'>  <font color=#9A32CD>统计日期： {sql_result1[0][-1]}</font>\n'
