...
def tp_check():
    ...
    sql = """
        ...
        where a.product_status in (1, 7)  -- 生产状态：1待处理、2处理中、3已发布、4不处理、7待修改 具体见数据字典
        and (
        ...
        and timestampdiff(MINUTE , a.update_time, now()) >= 1
...
def message_trans(data):
    ...
    if data:
        header = '## <font color=#FF3E96>新股任务留存时间异常</font>\n'
    else:
        header = '## <font color=#FF3E96>新股任务留存时间异常</font>\n'
    ...
    if data:
        ...
        for i in data:
            ...
            body += (f'+  【{task_pool}】中, 有<font color=#FF6347> **{task_count}** </font>条任务留存, '
                     f'最长留存<font color=#FF6347> **{task_delay}** </font>。\n')
    else:
        body += '+  暂无异常\n'

    footer = f'>  异常任务量：<font color=#FF34B3> **{count_task}** </font>'
