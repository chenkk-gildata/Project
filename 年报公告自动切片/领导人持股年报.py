...
def find_keywords(pdf_path):
    ...
    # 定义开始和结束关键词的正则表达式
    # 上深北开始关键词一致
    start_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]?[）)]?[、.．]?\s*(?:本行|公司)?董事(?:、监事)?[和及、]高级管理人员(?:的|基本)?情况', re.IGNORECASE | re.MULTILINE)

    if exchange_code == "bjs":
        end_pattern = re.compile(r'[（(]?[一二三四五12345][）)]?[、.．]?\s*变动情况', re.IGNORECASE | re.MULTILINE)
    elif exchange_code == "shs":
        # 上交所关键词
        end_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]?[）)]?[、.．]?\s*(公司)?董事(?:、监事)?[和、]高级管理人员(?:和核心技术人员)?变动情况', re.IGNORECASE | re.MULTILINE)
        remove_start_pattern = re.compile(r'主要工作经历', re.IGNORECASE | re.MULTILINE)
        remove_end_pattern = re.compile(r'董事(?:、监事)?[和、]高级管理人员(?:和核心技术人员)?(?:报酬|薪酬)情况', re.IGNORECASE | re.MULTILINE)
    else:
        # 深交所关键词
        end_pattern = re.compile(r'^[（(]?[\d二三四五六七八九][）)]?[、.．]?\s*(报告期内)?.*董事.*(履行职责的|履职|有关)情况', re.IGNORECASE | re.MULTILINE)
        # 24年报移除开始关键词
        #remove_start_pattern = re.compile(r'报告期.*任期内董事(?:、监事离任)?和高级管理人员解聘的情况', re.IGNORECASE)
        # 25年报移除开始关键词
        remove_start_pattern = re.compile(r'报告期是否存在任期内董事和高级管理人员离任的情况', re.IGNORECASE)
        remove_end_pattern = re.compile(r'董事(?:、监事)?[和及、]高级管理人员(?:和核心技术人员)?(?:的|年度)?(?:报酬|薪酬)情况', re.IGNORECASE)

    def get_search_rect(inst, keyword_type):
        """根据关键词类型智能调整搜索区域"""
        if keyword_type in ['高级管理人员', '变动情况', '主要工作经历', '报酬情况', '履行职责', '履职', '有关情况'
                            '监事离任', '高级管理人员离任', '薪酬情况']:
            return fitz.Rect(0, inst.y0 - 20, inst.x1 + 1500, inst.y1 + 50)
        else:
            # 默认搜索区域
            return fitz.Rect(0, inst.y0 - 50, inst.x1 + 1500, inst.y1 + 20)
    ...
    try:
        ...
        if exchange_code == "bjs":
            ...
            remove_start_keyword = ["监事离任", "高级管理人员离任"]
            remove_end_keyword = ["报酬情况", "薪酬情况"]
        ...
        # 定义搜索范围优先级
        search_ranges = [
            (15, 50),  # 第一优先级：覆盖72.7%的关键词
            (45, 70),  # 第二优先级：覆盖22.9%的关键词
            (65, 120)  # 第三优先级：覆盖4.4%的关键词
        ]
        ...
        for start_range, end_range in search_ranges:
            ...
            for page_num in range(actual_start - 1, actual_end):
                ...
                # 如果还没找到移除开始关键词，则搜索开始关键词(最后一个符合的)
...
def crop_page_after_keyword(pdf_path, keyword_info, output_dir, pagenum):
...
def process_pdf(pdf_path, keywords, output_dir):
    ...
    # 情况1：同时没有找到任何关键词，跳过处理
    if not start_info and not end_info and not remove_start_info and not remove_end_info:
        print(f"在文件 {os.path.basename(pdf_path)} 中未找到任何关键词，跳过处理")
        return None

    # 情况2：只找到了结束关键词，默认输出包含结束关键词页的前5页内容
    if not start_info and (end_info or remove_end_info):
        print(f"特殊处理：只找到结束关键词，输出包含结束关键词页的前5页内容")
        return crop_page_before_keyword(pdf_path, end_info, output_dir)

    # 情况3：只找到了开始关键词，默认输出包含开始关键词页的后5页内容
    if start_info and not end_info:
        pagenum = 2 if is_bjs else 4
        print(f"特殊处理：只找到开始关键词，输出包含开始关键词页的后{pagenum+1}页内容")
        return crop_page_after_keyword(pdf_path, start_info, output_dir, pagenum)
            ...
            if not is_bjs and remove_start_info and remove_end_info:
                ...
                for page_num in range(start_info['page_number'] - 1, end_info['page_number']):
                    ...
                    if (remove_start_info and remove_end_info and
                        ...
                        if page_num + 1 == remove_start_info['page_number'] and page_num + 1 != start_info[
                            'page_number']:
