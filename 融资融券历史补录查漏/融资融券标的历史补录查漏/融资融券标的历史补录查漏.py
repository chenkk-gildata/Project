...
def read_data_from_sql(start_date, end_date, info_source):
    """从SQL Server读取usrRZRQBDZQ表数据，根据日期区间和信息来源"""
    conn = get_db_connection()
    if conn:
        try:
            sql_query = f"""
            SELECT CONVERT(DATE,XXFBRQ) XXFBRQ,BDLB,ZQDM,ZQJC 
...
def optimize_compare_data(sql_df, excel_df):
    ...
    if 'tradeDate' in excel_df.columns:
        excel_df['tradeDate'] = pd.to_datetime(excel_df['tradeDate']).dt.date

    # 处理Excel数据：一条记录拆分为两条（融资和融券）
    excel_processed = []
    for _, row in excel_df.iterrows():
        # 融资记录 (BDLB=10)
        if pd.notna(row.get('finRate')):
            fin_row = {
                '日期': row['tradeDate'],
            ...
            excel_processed.append(fin_row)

        # 融券记录 (BDLB=20)
        if pd.notna(row.get('saleRate')):
            sale_row = {
                '日期': row['tradeDate'],
                'stockCode': row['stockCode'],
                'stockName': row['stockName'],
            ...
            excel_processed.append(sale_row)

    excel_processed_df = pd.DataFrame(excel_processed)

    if excel_processed_df.empty:
        print("Excel数据处理后为空")
        return pd.DataFrame()

    # 创建复合键用于比对
    sql_df['复合键'] = sql_df['XXFBRQ'].astype(str) + '_' + sql_df['ZQDM'].astype(str) + '_' + sql_df['BDLB'].astype(
        str)
    excel_processed_df['复合键'] = excel_processed_df['日期'].astype(str) + '_' + excel_processed_df[
        'stockCode'].astype(str) + '_' + excel_processed_df['BDLB'].astype(str)

    # 检查是否有重复的复合键
    sql_duplicates = sql_df['复合键'].duplicated().sum()
    excel_duplicates = excel_processed_df['复合键'].duplicated().sum()
    ...
    if excel_duplicates > 0:
        ...
        excel_processed_df = excel_processed_df.drop_duplicates(subset=['复合键'], keep='first')

    # 使用集合操作提高比对效率
    sql_keys = set(sql_df['复合键'].values)
    excel_keys = set(excel_processed_df['复合键'].values)

    print(f"数据库记录数: {len(sql_keys)}, Excel处理后的记录数: {len(excel_keys)}")
    ...
    # 获取Excel源文件的所有列名（除了复合键相关的临时列）
    original_excel_columns = [col for col in excel_df.columns if col not in ['复合键', '日期', 'BDLB', 'rate']]

    # 处理Excel中多出的记录（漏处理）
    print("处理Excel多出记录...")
    excel_dict = {row['复合键']: dict(row) for _, row in excel_processed_df.iterrows()}
    excel_original_dict = {(row['tradeDate'], row['stockCode']): dict(row) for _, row in excel_df.iterrows()}

    if missing_in_sql:
        for key in tqdm(missing_in_sql, desc="处理Excel多出记录"):
            if key in excel_dict:
                processed_row = excel_dict[key]
                date_obj = processed_row['日期']
                stock_code = processed_row['stockCode']

                # 获取原始Excel行的完整数据
                original_row = excel_original_dict.get((date_obj, stock_code), {})

                # 创建结果行，保留所有原始Excel列
                result_row = {}
                for col in original_excel_columns:
                    result_row[col] = original_row.get(col, '')

                # 添加备注信息
                bdlb = processed_row['BDLB']
                if bdlb == 10:
                    result_row['备注'] = '融资漏处理'
                elif bdlb == 20:
                    result_row['备注'] = '融券漏处理'
                else:
                    result_row['备注'] = '未知类型漏处理'
    ...
    if missing_in_excel:
        for key in tqdm(missing_in_excel, desc="处理数据库多出记录"):
            if key in sql_dict:
                sql_row = sql_dict[key]

                # 创建结果行，使用Excel格式
                result_row = {}

                # 填充基础信息
                result_row['tradeDate'] = sql_row['XXFBRQ']
                result_row['stockCode'] = sql_row['ZQDM']
                result_row['stockName'] = sql_row['ZQJC']
                result_row['marketName'] = ''
    ...
    if result_data:
        ...
        # 确保列顺序：先原始Excel列，然后是备注列
        output_columns = original_excel_columns + ['备注']
...
def save_result_to_excel(result_df, excel_filenames, start_date, end_date, info_source):
    ...
    try:
        # 按日期和股票代码正序排序
        if 'tradeDate' in result_df.columns and 'stockCode' in result_df.columns:
            result_df = result_df.sort_values(by=['tradeDate', 'stockCode'], ascending=[True, True])
...
if __name__ == "__main__":
    print("=" * 50 + "\n")
    print("usrRZRQBDZQ表融资融券可充抵查漏比对程序")
    print("\n" + "=" * 50)
