...
def find_keywords(pdf_path):
    ...
    if exchange_code == "bjs":
        start_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]*.*[）)]?[、.．]?\s*报告期期末的普通股股本结构、持股5%以上的股东或前十名股东情况\s*$', re.IGNORECASE | re.MULTILINE)
        end_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]*.*[）)]?[、.．]?\s*存续至本期的优先股股票相关情况\s*$', re.IGNORECASE | re.MULTILINE)
    else:
        start_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]*.*[）)]?[、.．]?\s*股东信息\s*$', re.IGNORECASE | re.MULTILINE)
        end_pattern = re.compile(r'^[（(]?[\d一二三四五六七八九]*.*[）)]?[、.．]?\s*其他(?:重要|提醒)事项\s*$', re.IGNORECASE | re.MULTILINE)
...
def crop_page_before_keyword(pdf_path, keyword_info, output_dir):
    ...
    try:
        ...
        for i in range(keyword_info['page_number'] - 3, keyword_info['page_number'] - 1):
...
def crop_page_after_keyword(pdf_path, keyword_info, output_dir):
    ...
    try:
        ...
        writer.add_page(cropped_page)
        for i in range(keyword_info['page_number'], keyword_info['page_number'] + 2):
            writer.add_page(reader.pages[i])
...
def process_gdbjjs_only(pdf_path, gdbjjs_info, output_dir):
    ...
    if start_info and end_info:
        ...
        if start_info['page_number'] == end_info['page_number']:
            print(f"开始关键词页码({start_info['page_number']})等于结束关键词页码({end_info['page_number']})，输出该页两关键词之间的内容")
