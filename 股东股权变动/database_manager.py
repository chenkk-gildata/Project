...
from config_ShareTransfer import db_config
...
class DatabaseConnectionPool:
    ...
    def _create_connection(self):
        ...
        try:
            ...
            get_db_logger().error(f"创建数据库连接失败: {e}")
            raise
        ...
        with self._lock:
            try:
                ...
                if len(self._pool) < self.max_connections:
                    ...
                    get_db_logger().warning(f"连接已失效，关闭连接: {e}")
                    try:
                        conn.close()
                    except:
                        pass
...
class DatabaseManager:
    ...
    def get_connection(self):
        ...
        try:
            ...
            get_db_logger().error(f"数据库操作失败: {e}")
            raise
        finally:
            if conn:
    ...
    def execute_query(self, query: str, params: tuple = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            get_db_logger().error(f"执行查询失败: {e}")
            raise
